...
from helper.fcn import fcn
from helper.skip import skip
...
def train_nnet_fun(option_var):
    ...
    for f in files_source:
        ...
        # optimizer
        optimizer = torch.optim.Adam([{'params': net.parameters()}, {'params': net_kernel.parameters(), 'lr': 1e-4}],
                                     lr=LR)
...
def deblurImage(self, image, kernel):
    torch.backends.cudnn.enabled = True
    torch.backends.cudnn.benchmark = True
    dtype = torch.cuda.FloatTensor
    ...
    input_depth = 8
    net_input = get_noise(input_depth, 'noise', (image.shape[0], image.shape[1])).type(dtype)
    ...
    n_k = 200
    net_input_kernel = get_noise(n_k, 'noise', (1, 1)).type(dtype)
    net_input_kernel.squeeze_()
    net_kernel = fcn(n_k, kernel.shape[0] * kernel.shape[1])
    net_kernel = net_kernel.type(dtype)

    # Load the trained models
    net.load_state_dict(torch.load("path/to/your/xnet_model.pth"))
    net_kernel.load_state_dict(torch.load("path/to/your/knet_model.pth"))

    # Convert image and kernel to torch tensors
    y = np_to_torch(image).type(dtype)
    kernel_tensor = np_to_torch(kernel).type(dtype)
    ...
    with torch.no_grad():
        out_x = net(net_input)
        out_k = net_kernel(net_input_kernel)
        out_k = out_k.view(-1, 1, kernel.shape[0], kernel.shape[1])
        out_y = nn.functional.conv2d(out_x, out_k, padding=0, bias=None)

    # Convert torch tensors to numpy arrays and return the deblurred image
    out_y_np = torch_to_np(out_y)
    return out_y_np.squeeze()
